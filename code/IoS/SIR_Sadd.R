...
SIR_S.add_deter$theta.names <- c("R0","D_inf","RR","Tadd", "Tint","Sadd")
SIR_S.add_deter$state.names <- c("S","I","R","Inc")
SIR_S.add_deter$simulate <- function(theta,init.state,times) {
  ...
  SIR_S.add <- function(time, state, parameters) {
  ...
  #############################################
  #when the time before Tadd
  if(Tadd < max(times)) {
    TaddBefore = times[1:Tadd]
  }
  else { print("Wrong, Tadd is larger than max(times).")  }
  
  init.state1 <- init.state
  # put incidence at 0 in init.state
  init.state1["Inc"] <- 0
  
  trajBefore <- as.data.frame(ode(init.state1, TaddBefore, SIR_S.add, theta, method = "ode45"))
  
  # compute incidence of each time interval
  trajBefore$Inc <- c(0, diff(trajBefore$Inc))
  
  #############################################
  #when the time between Tadd and Tadd+Tint
  if(Tint!=0) {
    #time=seq(from=Tadd+1, to=Tadd+Tint); S=rep(trajBefore$S[Tadd],times=Tint); I=rep(trajBefore$I[Tadd],times=Tint); 
    #R=rep(trajBefore$R[Tadd],times=Tint); Inc=rep(trajBefore$Inc[Tadd],times=Tint)
    #trajInt=cbind(time,S,I,R,Inc)
    TaddBetw = times[(Tadd+1):(Tadd+Tint)]
    init.state3 <- c('S' = trajBefore$S[Tadd]*(1 + Sadd1), 'I' = trajBefore$I[Tadd], 'R' = trajBefore$R[Tadd],'Inc' = trajBefore$Inc[Tadd])
    trajInt <- as.data.frame(ode(init.state3, TaddBetw, SIR_S.add, theta, method = "ode45"))
    trajInt$Inc <- c(init.state3["Inc"], diff(trajInt$Inc))
    
    trajBefore <- rbind(trajBefore,trajInt)
  }
  
  #############################################
  #when the time after Tadd+Tint
  if((Tadd+Tint) < max(times)){
    TaddAfter = times[(Tadd+Tint+1):max(times)]
  }
  else { print("Wrong, Tadd+Tint is larger than max(times).")  }
  
  init.state2 <- c('S' = trajBefore$S[Tadd+Tint]*(1 + Sadd2), 'I' = trajBefore$I[Tadd+Tint], 'R' = trajBefore$R[Tadd+Tint],'Inc' = trajBefore$Inc[Tadd+Tint])
  
  trajAfter <- as.data.frame(ode(init.state2, TaddAfter, SIR_S.add, theta, method = "ode45"))
  # compute incidence of each time interval
...
#attack rate
sum(obs.traj$Inc)/(S0 + S0add)
###########################################################
####plot designed by Bo Xu
par(mar=c(5, 4, 2, 4))
plot(x = epi_time[-c(length(epi_time), length(epi_time)-1 )], y = obs.traj$Inc, type = "l", lty=1 ,col = "red", lwd=2, xlab = "Day", ylab = "Number of cases")
lines(x = epi_time[-c(length(epi_time), length(epi_time)-1 )], y = obs.traj$obs, type = "l", lty=1,col = "black",lwd=2)
#abline(v = init.theta[["Tadd"]], type = "l", lty=2, col="blue",lwd=0.5)
#abline(v = init.theta[["Tadd"]] + init.theta[["Tint"]]-1, type = "l", lty=2, col="blue",lwd=0.5)
#plot(x = epi_time[-c(length(epi_time), length(epi_time)-1 )], y = obs.traj$S, type = "l", lty=1 ,col = "dark gray", lwd=3, xlab = "Day", ylab = "Number of susceptible")
legend("topright", legend = c("Incidence","Incidence observation","Proportion of S added"),bty = "n", lty = c(1,1,2), lwd = c(2,2,2),col = c("red","black","dark gray") )

par(new=TRUE)
plot(x=epi_time[-c(length(epi_time), length(epi_time)-1 )], y=obs.traj$Sadd, type = "s", lty=2, col="dark gray", lwd=2, yaxt="n", ylab = "",xaxt="n", xlab = "" )
axis(side = 4)
# add a title for the right axis 
mtext("Proportion of susceptible added", side=4, line=2.5, cex.lab=1, las=0, col="black")
