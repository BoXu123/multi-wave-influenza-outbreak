...
begin2 = 0
pS1 = seq(begin1, 2, by=step1) #pS1是为S1设定的取值范围
pS2 = seq(begin2, 3, by=step2) #pS2是为S2设定的取值范围
PNmat = matrix(99, nrow = length(pS1), ncol = length(pS2)) #存储number of waves
ATmat = matrix(0, nrow = length(pS1), ncol = length(pS2)) #存储attack rate
epi_time = c(1:100)
...
for(s1 in pS1){
  for(s2 in pS2){
    ...
    init.state <- c('S' = S0, 'I' = 1, 'R' = 5,'Inc' = 0)
    init.theta <- c(R0 = 2.2, D_inf = 3.3,D_lat =0, RR = 0.75, Tadd = Tadd, Tint = Tint, Sadd1 = s1, Sadd2 = s2)
    obs.traj <- rTrajObs(fitmodel = SIR_S.add_deter, theta = init.theta, init.state = init.state, times = epi_time)
    #The total number of people adding into the population of the region
    S0add = s1 * obs.traj$S[Tadd] + s2 * obs.traj$S[Tadd+Tint]
    #"Sadd" is the proportion of susceptible people at the time of Tadd adding to the susceptible compartment.
    #obs.traj$Sadd <- c(rep(0, times=init.theta[["Tadd"]]-1), rep(init.theta[["Sadd1"]], times=init.theta[["Tint"]]),
                      # rep(init.theta[["Sadd2"]], times=length(epi_time)-init.theta[["Tadd"]]+1-init.theta[["Tint"]]))
    obs.traj <- obs.traj[-c((init.theta[["Tadd"]] + 1), (init.theta[["Tadd"]] + init.theta[["Tint"]] + 1)),]
    
    Nreal = (S0 + S0add)
    #attack rate
    AT = sum(obs.traj$Inc)/Nreal
    #ATmat[-4+(r01/step1),-4+(r02/step2)] = AT
    A1 = ((s1-begin1)/step1)+1
    B1 = ((s2-begin2)/step2)+1
    #print(ATmat[A1,B1])
    ATmat[A1,B1] <- AT
    ...
    PNmat[((s1-begin1)/step1)+1,((s2-begin2)/step2)+1] <- peakNum
...
myImagePlot(PNmat, xLabels=pS2,yLabels=pS1, title=c("Number of waves"), zlim=c(1,2)) 
#myImagePlot(t(PNmat), xLabels=pC11,yLabels=pA22, title=c("Number of waves"), zlim=c(1,2)) 
myImagePlot(ATmat, xLabels=pS2,yLabels=pS1, title=c("Attack rate"), zlim=c(min(ATmat),max(ATmat))) 
...
colnames(two_param)=c('s1','s2')
for(i in c(1:Num_two_param)) {
  if((ind2[i] %% nrow(PNmat)) == 0) {
    two_param[i,1] = max(pS1)
    two_param[i,2] = begin2 + step2 * (ind2[i] %/% nrow(PNmat) - 1)
  }
  else {
    two_param[i,1] = begin1 + step1 * ((ind2[i] %% nrow(PNmat)) - 1) # "%%"表示取余。计算得到s1
    two_param[i,2] = begin2 + step2 * (ind2[i] %/% nrow(PNmat)) # "%/%"表示整除。计算得到s2
  }
}

#保存到本地
write.csv(PNmat,file = "E:/科研/paper/bi-modal/figures/2Susceptible-Adding/PNmat_s1&s2.csv") #存储number of waves
write.csv(ATmat, file = "E:/科研/paper/bi-modal/figures/2Susceptible-Adding/ATmat_s1&s2.csv") #存储attack rate
write.csv(two_param, file = "E:/科研/paper/bi-modal/figures/2Susceptible-Adding/twoWaveParam_s1&s2.csv") #存储 能得到双峰的参数组合
...
begin2 = 2 #Tadd需从2起始
pS1 = seq(begin1, 2, by=step1) #pS1是为S1设定的取值范围
pTadd = seq(begin2, 80, by=step2) #pTadd是为Tadd设定的取值范围
PNmat = matrix(99, nrow = length(pS1), ncol = length(pTadd)) #存储number of waves
ATmat = matrix(0, nrow = length(pS1), ncol = length(pTadd)) #存储attack rate
epi_time = c(1:100)
...
for(s1 in pS1){
  for(Tadd in pTadd){
    ...
    PNmat[((s1-begin1)/step1)+1,((Tadd-begin2)/step2)+1] <- peakNum
...
source("http://www.phaget4.org/R/myImagePlot.R")
#setwd("E:/科研/paper/bi-modal/figures")
myImagePlot(PNmat, xLabels=pTadd,yLabels=pS1, title=c("Number of waves"), zlim=c(1,2)) 
#myImagePlot(t(PNmat), xLabels=pC11,yLabels=pA22, title=c("Number of waves"), zlim=c(1,2)) 
myImagePlot(ATmat, xLabels=pTadd,yLabels=pS1, title=c("Attack rate"), zlim=c(min(ATmat),max(ATmat))) 
...
for(i in c(1:Num_two_param)) {
  if((ind2[i] %% nrow(PNmat)) == 0) {
    ...
    two_param[i,1] = begin1 + step1 * ((ind2[i] %% nrow(PNmat)) - 1) # "%%"表示取余。计算得到s1
    two_param[i,2] = begin2 + step2 * (ind2[i] %/% nrow(PNmat)) # "%/%"表示整除。计算得到Tadd
  }
}

#保存到本地
write.csv(PNmat,file = "E:/科研/paper/bi-modal/figures/2Susceptible-Adding/0604/PNmat_s1&Tadd.csv") #存储number of waves
write.csv(ATmat, file = "E:/科研/paper/bi-modal/figures/2Susceptible-Adding/0604/ATmat_s1&Tadd.csv") #存储attack rate
write.csv(two_param, file = "E:/科研/paper/bi-modal/figures/2Susceptible-Adding/0604/twoWaveParam_s1&Tadd.csv") #存储 能得到双峰的参数组合
...
begin1 = 0 #begin 必须是零，否则会出错！！！
begin2 = 2 #Tadd需从2起始
pS2 = seq(begin1, 3, by=step1) #pS2是为S2设定的取值范围
pTadd = seq(begin2, 80, by=step2) #pTadd是为Tadd设定的取值范围
PNmat = matrix(99, nrow = length(pS2), ncol = length(pTadd)) #存储number of waves
ATmat = matrix(0, nrow = length(pS2), ncol = length(pTadd)) #存储attack rate
epi_time = c(1:100)
...
for(s2 in pS2){
  for(Tadd in pTadd){
    ...
    PNmat[((s2-begin1)/step1)+1,((Tadd-begin2)/step2)+1] <- peakNum
...
source("http://www.phaget4.org/R/myImagePlot.R")
#setwd("E:/科研/paper/bi-modal/figures")
myImagePlot(PNmat, xLabels=pTadd,yLabels=pS2, title=c("Number of waves"), zlim=c(1,2)) 
#myImagePlot(t(PNmat), xLabels=pC11,yLabels=pA22, title=c("Number of waves"), zlim=c(1,2)) 
myImagePlot(ATmat, xLabels=pTadd,yLabels=pS2, title=c("Attack rate"), zlim=c(min(ATmat),max(ATmat))) 
...
Num_two_param = length(ind2) #能得到双峰的参数组合的数目
two_param = matrix(data = 0, nrow = Num_two_param, ncol = 2) #存放 能得到双峰的参数组合 的矩阵
colnames(two_param)=c('s2','Tadd')
for(i in c(1:Num_two_param)) {
  if((ind2[i] %% nrow(PNmat)) == 0) {
    two_param[i,1] = max(pS2)
    two_param[i,2] = begin2 + step2 * (ind2[i] %/% nrow(PNmat) - 1)
  }
  else {
    two_param[i,1] = begin1 + step1 * ((ind2[i] %% nrow(PNmat)) - 1) # "%%"表示取余。计算得到s2
    two_param[i,2] = begin2 + step2 * (ind2[i] %/% nrow(PNmat)) # "%/%"表示整除。计算得到Tadd
  }
}

#保存到本地
write.csv(PNmat,file = "E:/科研/paper/bi-modal/figures/2Susceptible-Adding/0604/PNmat_s2&Tadd.csv") #存储number of waves
write.csv(ATmat, file = "E:/科研/paper/bi-modal/figures/2Susceptible-Adding/0604/ATmat_s2&Tadd.csv") #存储attack rate
write.csv(two_param, file = "E:/科研/paper/bi-modal/figures/2Susceptible-Adding/0604/twoWaveParam_s2&Tadd.csv") #存储 能得到双峰的参数组合
