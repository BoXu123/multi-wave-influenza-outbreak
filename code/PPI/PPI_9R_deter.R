...
##################model constrution
data("SEIT4L_deter")
PPI_9R_deter <- SEIT4L_deter
PPI_9R_deter$state.names <- c("S","E","I","R1","R2","R3","R4","R5","R6","R7","R8","R9","L","RL","Inc")
PPI_9R_deter$theta.names <- c("R0","D_lat","D_inf","D_imm","sigma","rho")
PPI_9R_deter$simulate <- function(theta,init.state,times) {
  
  PPI_9R_ode <- function(time, state, theta) {
    ...
    N <- S + E + I + R1 + R2 + R3 + R4 + R5 + R6 + R7 + R8 + R9 + L
    
    dS <- -beta*S*I/N
    dE <- beta*S*I/N - epsilon*E + sigma*beta*L*I/N
    dI <- epsilon*E - nu*I
    dR1 <- nu*I - 9*gamma*R1
    dR2 <- 9*gamma*R1 - 9*gamma*R2
    dR3 <- 9*gamma*R2 - 9*gamma*R3
    dR4 <- 9*gamma*R3 - 9*gamma*R4
    dR5 <- 9*gamma*R4 - 9*gamma*R5
    dR6 <- 9*gamma*R5 - 9*gamma*R6
    dR7 <- 9*gamma*R6 - 9*gamma*R7
    dR8 <- 9*gamma*R7 - 9*gamma*R8
    dR9 <- 9*gamma*R8 - 9*gamma*R9
    dL <- 9*gamma*R9 - sigma*beta*L*I/N
    dInc <- epsilon*E
    dRL <- 9*gamma*R9
    
    return(list(c(dS,dE,dI,dR1,dR2,dR3,dR4,dR5,dR6,dR7,dR8,dR9,dL,dRL,dInc)))
  }
  ...
  traj <- as.data.frame(ode(init.state, times, PPI_9R_ode, theta, method = "ode45"))
  
  # compute incidence of each time interval
  traj$Inc <- c(0, diff(traj$Inc))
  traj$RL <- c(0, diff(traj$RL))
...
init.theta <- c(R0 = 6.9, D_lat = 1.8, D_inf = 1.0, D_imm = 5.7, sigma = 0.29, rho = 0.68) #只有sigma小于等于0.16时，Incidence曲线在第100天之前才会出现递减趋势
epi_time = c(1:100)
...
pdf(file = "PPI_9R.pdf", width = 7.5, height = 5)
...
legend("topright", legend = c("incidence","observation of incidence"),lty = c(2,1), lwd = c(3,1),col = c("black","red") )
...
thre1 = 0.5
thre2 = 6 #W2的最大值为0.194970448，thre2要大于1/0.194970448=5.13才能使peakNum为2
