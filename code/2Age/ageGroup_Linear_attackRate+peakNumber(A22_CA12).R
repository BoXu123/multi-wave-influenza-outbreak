...
step1 = 5 #步长
step2 = 1
pA22 = seq(0, 150, by=step1)#A22表示成人一年中有效接触次数,A对应adults，pA22表示设定的A22的取值范围
pCA12 = seq(0, 30, by=step2)#CA12表示成人和孩子之间一年中的有效接触次数，C对应children，A对应Adults,pCA12表示设定的CA12的取值范围
PNmat = matrix(0, nrow = length(pA22), ncol = length(pCA12)) #存储number of waves
ATmat = matrix(0, nrow = length(pA22), ncol = length(pCA12)) #存储attack rate
C11 = 170
epi_time = c(1:200)
N = 27700
...
for(A22 in pA22){
  for(CA12 in pCA12){
    ...
    ATmat[1+A22/step1,1+CA12/step2] = AT
    ...
    #Number of peak
    #PN = cbind(PN,peakNum)
    PNmat[1+A22/step1,1+CA12/step2] = peakNum
  }
...
myImagePlot(PNmat, xLabels=pCA12,yLabels=pA22, title=c("Number of waves"), zlim=c(1,2)) 
#myImagePlot(t(PNmat), xLabels=pC11,yLabels=pA22, title=c("Number of waves"), zlim=c(1,2)) 
myImagePlot(ATmat, xLabels=pCA12,yLabels=pA22, title=c("Attack rate"), zlim=c(min(ATmat),max(ATmat))) 
...
colnames(two_param)=c('A22_Adult','CA12_AdultChild')
for(i in c(1:Num_two_param)) {
  if((ind2[i] %% nrow(PNmat)) == 0) {
    two_param[i,1] = max(pA22)
    two_param[i,2] = step2 * (ind2[i] %/% nrow(PNmat) - 1)
  }
  else {
    two_param[i,1] = step1 * ((ind2[i] %% nrow(PNmat)) - 1) # "%%"表示取余。计算得到C11
    two_param[i,2] = step2 * (ind2[i] %/% nrow(PNmat)) # "%/%"表示整除。计算得到CA12
  }
}

#保存到本地
write.csv(PNmat,file = "E:/科研/paper/bi-modal/figures/7Two-Age-Group/PNmat_A22&CA12.csv") #存储number of waves
write.csv(ATmat, file = "E:/科研/paper/bi-modal/figures/7Two-Age-Group/ATmat_A22&CA12.csv") #存储attack rate
write.csv(two_param, file = "E:/科研/paper/bi-modal/figures/7Two-Age-Group/twoWaveParam_A22&CA12.csv") #存储 能得到双峰的参数组合
...
CA12 = 6 #固定C11为245，取CA12为50、80、105、125
A22 = 50
epi_time = c(1:200)
N = 27700
init.state <- c('N'=N,'S1a' = 0.3, 'E1a'=0, 'I1a' = 0.0012, 'R1a' = 0,'S2a' = 0.22, 'E2a'=0, 'I2a' = 0.001, 'R2a' = 0.02,'Inc1' = 0,'Inc2' = 0,'Inc' = 0)
...
legend("topleft", legend = c("CA12 = 6","C11 = 170"),bty = "n")
legend("topright", legend = c("A22 = 50","A22 = 80","A22 = 105","A22 = 125"),bty = "n", lty = c(1,1,1), lwd = c(2,2,2),col = c("#FFFF00","#7F7F80","#3333CC","#0000FF") )
legend(x=125, y=140, legend = c("PN=2","PN=1.5","PN=1.2","PN=1"),bty = "n")
