...
step1 = 5 #步长
step2 = 1
pC11 = seq(0, 300, by=step1)#C11表示孩子们一年中有效接触次数,C对应children，pC11表示设定的C11的取值范围
pCA12 = seq(0, 30, by=step2)#CA12表示成人和孩子之间一年中的有效接触次数，C对应children，A对应Adults,pCA12表示设定的CA12的取值范围
PNmat = matrix(0, nrow = length(pC11), ncol = length(pCA12)) #存储number of waves
ATmat = matrix(0, nrow = length(pC11), ncol = length(pCA12)) #存储attack rate
A22 = 53
epi_time = c(1:200)
N = 27700
...
#保存到本地
write.csv(PNmat,file = "E:/科研/paper/bi-modal/figures/7Two-Age-Group/PNmat_C11&CA12.csv") #存储number of waves
write.csv(ATmat, file = "E:/科研/paper/bi-modal/figures/7Two-Age-Group/ATmat_C11&CA12.csv") #存储attack rate
write.csv(two_param, file = "E:/科研/paper/bi-modal/figures/7Two-Age-Group/twoWaveParam_C11&CA12.csv") #存储 能得到双峰的参数组合
...
C11 = 245 #固定C11为245，取CA12为2、8、12、24
CA12 = 2
epi_time = c(1:200)
N = 27700
init.state <- c('N'=N,'S1a' = 0.3, 'E1a'=0, 'I1a' = 0.0012, 'R1a' = 0,'S2a' = 0.22, 'E2a'=0, 'I2a' = 0.001, 'R2a' = 0.02,'Inc1' = 0,'Inc2' = 0,'Inc' = 0)
