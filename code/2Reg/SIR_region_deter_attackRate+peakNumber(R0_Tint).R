...
begin2 = 0
pR0 = seq(begin1, 9, by=step1) #pR0是为R0设定的取值范围
pTint = seq(begin2, 50, by=step2) #pTint是为Tint设定的取值范围
PNmat = matrix(99, nrow = length(pR0), ncol = length(pTint)) #存储number of waves
ATmat = matrix(0, nrow = length(pR0), ncol = length(pTint)) #存储attack rate
epi_time = c(1:100)
...
for(r0 in pR0){
  for(tint in pTint){
    ...
    init.state <- c('S2' = N/lam, 'I2' = 5, 'R2' = 1,'Inc2' = 0,'S1' = N, 'I1' = 2, 'R1' = 3,'Inc1' = 0)
    init.theta <- c(R0 = r0, D_inf = 3.6,D_lat =0, RR = 0.75, T=tint)
    ...
    PNmat[((r0-begin1)/step1)+1,((tint-begin2)/step2)+1] <- peakNum
  }
}
...
myImagePlot(PNmat, xLabels=pTint,yLabels=pR0, title=c("Number of waves"), zlim=c(1,2)) 
#myImagePlot(t(PNmat), xLabels=pC11,yLabels=pA22, title=c("Number of waves"), zlim=c(1,2)) 
myImagePlot(ATmat, xLabels=pTint,yLabels=pR0, title=c("Attack rate"), zlim=c(min(ATmat),max(ATmat))) 
...
for(i in c(1:Num_two_param)) {
  if((ind2[i] %% nrow(PNmat)) == 0) {
    ...
    two_param[i,2] = begin2 + step2 * (ind2[i] %/% nrow(PNmat)) # "%/%"表示整除。计算得到Tint
  }
}

#保存到本地
write.csv(PNmat,file = "E:/科研/paper/bi-modal/figures/1Two-Regions-Combined/PNmat_R0&Tint.csv") #存储number of waves
write.csv(ATmat, file = "E:/科研/paper/bi-modal/figures/1Two-Regions-Combined/ATmat_R0&Tint.csv") #存储attack rate
write.csv(two_param, file = "E:/科研/paper/bi-modal/figures/1Two-Regions-Combined/twoWaveParam_R0&Tint.csv") #存储 能得到双峰的参数组合
