...
SIR_region_deter$simulate <- function(theta,init.state,times) {
  ...
  SIR_region1 <- function(time, state, parameters) {# 若T==0，则两区域同时开始流行疫情
  ...
  SIR_region2 <- function(time, state, parameters) {# 若T!=0，则两区域疫情流行开始时间错开T天
  ...
  T <- theta[["T"]] #T是两区域疫情流行开始时间的间隔天数
  if(T==0) { # 若T==0，则两区域同时开始流行疫情
    ###以下4行（第74行到第77行）是错误的
...
init.theta <- c(R0 = 3.2, D_inf = 3.6,D_lat =0, RR = 0.75, T=15) ##若T=0，画图时要用第310行到第315行的代码
#改变time interval T和basic reproductino number R0
