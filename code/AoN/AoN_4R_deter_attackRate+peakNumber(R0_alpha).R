...
palpha = seq(begin2, 1, by=step2)
PNmat = matrix(99, nrow = length(pR0), ncol = length(palpha)) #存储number of waves
ATmat = matrix(99, nrow = length(pR0), ncol = length(palpha)) #存储attack rate
epi_time = c(1:100)
...
for(r0 in pR0){
  ...
  for(alph in palpha){
    ...
    init.state <- c('S' = 27700, 'E' = 0, 'I' = 1, 'R1' = 0, 'R2' = 0,'R3' = 0, 'R4' = 0, 'L'= 6,'RS'=0,'Inc' = 0)
    init.theta <- c(R0 = r0, D_lat = 2.1, D_inf = 2.4, D_imm = 11.6, alpha = alph, rho = 0.71)
    obs.traj <- rTrajObs(fitmodel = AoN_4R_deter, theta = init.theta, init.state = init.state, times = epi_time)
    
    Nreal = init.state[["S"]]+sum(obs.traj$RS) #RS表示由R状态转变为S状态的人数，这部分人再次成为病毒的易感者
    AT = sum(obs.traj$Inc)/Nreal
    A1 = ((r0-begin1)/step1)+1
    B1 = ((alph-begin2)/step2)+1
    #print(ATmat[A1,B1])
    ATmat[A1,B1] <- AT
    ...
    PNmat[((r0-begin1)/step1)+1,((alph-begin2)/step2)+1] <- peakNum
...
myImagePlot(PNmat, xLabels=palpha, yLabels=pR0, title=c("Number of waves"), zlim=c(1,2)) 
#myImagePlot(t(PNmat), xLabels=pC11,yLabels=pA22, title=c("Number of waves"), zlim=c(1,2)) 
myImagePlot(ATmat, xLabels=palpha, yLabels=pR0, title=c("Attack rate"), zlim=c(min(ATmat),max(ATmat))) 
...
#保存到本地
write.csv(PNmat,file = "E:/科研/paper/bi-modal/figures/11All-or-Nothing (AoN)/PNmat_R0&alpha.csv") #存储number of waves
write.csv(ATmat, file = "E:/科研/paper/bi-modal/figures/11All-or-Nothing (AoN)/ATmat_RR0&alpha.csv") #存储attack rate
write.csv(two_param, file = "E:/科研/paper/bi-modal/figures/11All-or-Nothing (AoN)/twoWaveParam_R0&alpha.csv") #存储 能得到双峰的参数组合

#########################################################################################################################################
#########################################################################################################################################
######plot epidemic curves (Begin)
alph = 0.2 #固定alph为0.2，R0取值为2.5、3.5、6.5
r0 = 2.5

epi_time = c(1:100)
init.state <- c('S' = 27700, 'E' = 0, 'I' = 1, 'R1' = 0, 'R2' = 0,'R3' = 0, 'R4' = 0, 'L'= 6,'Inc' = 0,'RS'=0)
init.theta <- c(R0 = r0, D_lat = 2.1, D_inf = 2.4, D_imm = 11.6, alpha = alph, rho = 0.71)
obs.traj <- rTrajObs(fitmodel = AoN_4R_deter, theta = init.theta, init.state = init.state, times = epi_time)
...
legend("topleft", legend = c("α = 0.2"),bty = "n")
legend("topright", legend = c("R0 = 2.5","R0 = 3.5","R0 = 6.5"),bty = "n", lty = c(1,1,1), lwd = c(2,2,2),col = c("#0000FF","#7F7F80","#FFFF00") )
legend(x=60, y=3000, legend = c("PN=1","PN=1.5","PN=2"),bty = "n")
##PN为1的对应RGB颜色为(0,0,255),PN为1.2的对应RGB颜色为(51,51,204),PN为1.5的对应RGB颜色为(127,127,128),PN为2的对应RGB颜色为(255,255,0)
##PN为1的对应颜色为"#0000FF"，PN为1.2的对应颜色为"#3333CC"，PN为1.5的对应颜色为"#7F7F80"，PN为2的对应颜色为"#FFFF00"
